...
export default function AppNavbar() {
  ...
  useEffect(() => {
    setUserRole(sessionStorage.getItem('forge_user_role'));
    setUsername(sessionStorage.getItem('forge_username'));
  }, [location.pathname]);

  const handleLogout = () => {
    sessionStorage.removeItem('forge_jwt_token');
    sessionStorage.removeItem('forge_user_role');
    sessionStorage.removeItem('forge_username');
    setUserRole(null);
    setUsername(null);
    navigate('/login');
  };
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            ...
            {userRole ? (
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full text-xs">
                <span className="font-semibold text-orange-400 font-mono">{username}</span>
