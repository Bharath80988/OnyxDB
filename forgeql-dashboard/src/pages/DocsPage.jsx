                ...
                {frameworks.map((framework) => (
                  <div key={framework.id} className="mb-4">
                    <h4 className="text-xs font-semibold text-white/80 mb-2 px-3 flex items-center gap-1.5">
                      <Code2 className="w-3.5 h-3.5 text-orange-400" /> {framework.title}
                    </h4>
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3 py-1.5 rounded-r-lg text-xs transition-colors ${
                              activeSection === sectionId ? 'text-orange-400 font-bold bg-orange-400/10' : 'text-white/50 hover:text-white'
                            }`}
                          >
                            {chapter.title}
                          </button>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <section id="intro" className="mb-16 pt-4">
              <h1 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight">
                Welcome to <span className="text-orange-400">ForgeQL Documentation</span>
              </h1>
              <p className="text-base text-white/70 leading-relaxed mb-6">
                ForgeQL is an operational document-oriented database built on B+ Trees, mmap zero-copy memory, and HNSW vector search. It speaks native JSON over HTTP (port 8080) and zero-copy OWP binary over TCP (port 8081).
              </p>
              
              <div className="bg-[#121214] border border-white/10 rounded-2xl p-8 shadow-xl grid md:grid-cols-2 gap-8">
                <div>
                  <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                    <Zap className="w-5 h-5 text-orange-400" /> Why ForgeQL?
                  </h3>
                ...
                <div className="bg-black/80 border border-white/10 rounded-xl p-4 font-mono text-xs text-orange-400 overflow-hidden relative">
            ...
            <section id="philosophy" className="mb-24 scroll-mt-24">
              ...
              <div className="space-y-4 text-sm text-white/70 leading-relaxed">
                <p>
                  ForgeQL combines low-level operating system optimizations with modern vector AI capabilities.
                </p>
                <p>
                  We built ForgeQL on top of <strong>B+ Trees</strong> to ensure that direct lookups operate in logarithmic time, while disk I/O is minimized through `MmapStorageManager` and direct off-heap direct byte buffers.
                </p>
                <div className="bg-white/5 border border-white/10 rounded-xl p-6 my-6">
                  <h4 className="font-bold text-orange-400 mb-2 flex items-center gap-2"><Server className="w-5 h-5"/> Multi-Reactor TCP &amp; REST Server</h4>
                  <p className="text-xs text-white/70">ForgeQL runs as an independent executable or embedded dependency. It listens on port 8080 for HTTP REST queries and port 8081 for zero-copy TCP socket framing.</p>
                </div>
              </div>
            </section>
              ...
              {frameworks.map((framework) => (
                <div key={framework.id} className="pt-8">
                  <h2 className="text-3xl font-bold mb-8 text-white flex items-center gap-3">
                    <Code2 className="w-7 h-7 text-orange-400" /> {framework.title}
                  </h2>
