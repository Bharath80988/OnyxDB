...
const FEATURES = [
  {
    ...
    longDesc: "By leveraging Operating System virtual memory page mapping (FileChannel.map), ForgeQL bypasses Java garbage collector pauses and user-space buffer copying during heavy write operations.",
    tag: "STORAGE CORE",
...
// Direct page slot lookup in off-heap virtual memory`,
  ...
  },
  {
    ...
    codeExample: `// FQL Primary Key Lookup
{
  "fql": "GET users 101"
}`,
    examplePayload: { fql: "GET users 101" }
  },
  {
    id: "hnsw",
    icon: Network,
...
[0x00, 0x00, 0x00, 0x2A] -> 4-Byte Length Payload`,
    examplePayload: { fql: "GET telemetry 9901" }
  },
  {
    id: "relational",
    icon: GitBranch,
...
{
  "fql": "EXPLAIN FIND orders WHERE amount > 500"
}`,
    examplePayload: { fql: "EXPLAIN FIND orders WHERE amount > 500" }
  },
  {
    id: "wal",
    icon: Layers,
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 text-orange-400 text-xs font-mono mb-4 border border-white/10">
            <Zap className="w-3.5 h-3.5" /> ForgeQL Capabilities &amp; Specifications
          </div>
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-orange-400 group-hover:bg-orange-400/10 transition-colors">
              ...
              <div className="pt-4 mt-6 border-t border-white/5 text-xs font-mono text-orange-400/80 flex items-center justify-between">
                <span>{feat.file}</span>
