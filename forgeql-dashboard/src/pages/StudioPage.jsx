...
export default function StudioPage() {
  ...
  const handleRunQuery = async (queryPayload) => {
    setLoading(true);
    setResult('Executing payload against ForgeQL engine...');
    try {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-white/10 pb-4 mb-6 gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
              <Database className="w-6 h-6 text-orange-400" /> Forge Studio <span className="text-xs px-2.5 py-0.5 rounded-full bg-orange-500/20 text-orange-400 font-mono">v4.0.0 IDE</span>
            </h1>
            ...
            {tab === 'json' && (
              <div className="p-6 h-full flex flex-col gap-4 overflow-auto">
                ...
                <textarea
                  ...
                  placeholder='{"action": "insert", "table": "users", "data": {"id": 1, "name": "ForgeQL"}}'
                  className="w-full h-56 bg-black/60 border border-white/10 rounded-xl p-4 font-mono text-xs text-orange-400 focus:outline-none focus:border-white/30 resize-none"
                  spellCheck={false}
                />
            ...
            {tab === 'create' && (
              <div className="p-8 max-w-2xl mx-auto w-full flex flex-col justify-center">
                <h3 className="text-xl font-bold mb-2 flex items-center gap-2">
                  <Plus className="w-5 h-5 text-orange-400" /> Create New Table &amp; Relational Schema
                </h3>
                ...
                {createMsg && (
                  <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-3 text-xs text-orange-400 mb-6">
                    {createMsg}
                  </div>
                )}
