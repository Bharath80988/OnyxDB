...
/**
 * Non-Blocking Native TCP Socket Server (`OnyxNativeSocketServer`).
 * Bypasses HTTP REST servlet overhead running on port `8081` with Round-Robin worker load balancing.
 */
public class OnyxNativeSocketServer implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(OnyxNativeSocketServer.class);
    ...
    public OnyxNativeSocketServer(int port, ExecutionEngine executionEngine) throws IOException {
        this.port = port;
    ...
    public void run() {
        try {
            ...
            log.info("🚀 OnyxNativeSocketServer (Zero-Copy Non-Blocking NIO) running on port {}", port);
            ...
            if (running) {
                log.error("Error in OnyxNativeSocketServer acceptor loop", e);
            }
        } finally {
            stop();
        }
    ...
    public void stop() {
        ...
        log.info("OnyxNativeSocketServer stopped.");
    }
}
