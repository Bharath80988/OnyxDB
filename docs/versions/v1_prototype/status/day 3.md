...
- **Python SDK / Terminal CLI:** Created a Python SDK (`sdks/python/forgeql.py`) to execute queries natively from the terminal.
...
- **Orchestration:** Created `docker-compose.yml` to run ForgeQL alongside a Redis cache container and a Celery worker template.
...
- **JitPack Documentation:** Added instructions in the `README.md` for Java developers to embed the pure `forgeql-core` B+ Tree engine into their own applications.
