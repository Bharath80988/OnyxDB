...
import com.forgeql.core.index.BTreeManager;
import com.forgeql.core.index.SecondaryBTreeIndex;
import com.forgeql.core.index.hnsw.HnswIndex;
import com.forgeql.core.schema.ForeignKeyConstraint;
import com.forgeql.core.schema.SchemaManager;
import com.forgeql.core.storage.BufferPool;
import com.forgeql.core.storage.StorageManager;
import com.forgeql.core.wal.WriteAheadLog;
...
/**
 * Central query execution engine for ForgeQL.
 *
 * <p>Routes structured JSON query nodes and FQL string queries across multiple dynamically
...
public class ExecutionEngine {
     ...
     * Used by the Forge CLI for {@code SHOW TABLES} meta-command.
     *
    ...
    public List<String> execute(String fqlQuery) throws Exception {
        if (fqlQuery == null || fqlQuery.trim().isEmpty()) {
            throw new IllegalArgumentException("Query string cannot be empty.");
        }
        return execute(parseOqsToQueryNode(fqlQuery));
    }

    public List<String> execute(Map<String, Object> queryNode) throws Exception {
        if (queryNode.containsKey("fql")) {
            return execute((String) queryNode.get("fql"));
        }
        ...
        if (action != null && (action.equalsIgnoreCase("fql") || action.equalsIgnoreCase("raw"))) {
            return execute((String) queryNode.get("query"));
        }
        ...
        if (action == null) {
            throw new IllegalArgumentException("Query must contain an 'action' or 'fql' field");
        }
        if (table == null) {
    ...
    private List<String> executeExplain(String tableName, BTreeManager db, Map<String, Object> queryNode) throws IOException {
        TableStats stats = tableStatsMap.computeIfAbsent(tableName, TableStats::new);
    ...
    /**
     * Parses human-readable Forge Query Syntax (FQL) strings into structured query nodes.
     */
    public Map<String, Object> parseOqsToQueryNode(String query) {
        if (query == null || query.trim().isEmpty()) {
            throw new IllegalArgumentException("FQL query cannot be empty");
        }
        ...
        throw new IllegalArgumentException("Unrecognized Forge Query Syntax: " + query);
    }
