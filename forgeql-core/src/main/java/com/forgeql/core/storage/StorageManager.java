...
public class StorageManager {
    ...
    /**
     * Defragments a target page in storage to reclaim internal fragmented bytes.
     *
     * @param page Target page to defragment.
     * @return Number of bytes reclaimed.
     */
    public int compactPage(Page page) throws IOException {
        int reclaimed = page.defrag();
        if (reclaimed > 0) {
            writePage(page);
            log.info("Compacted page {} reclaiming {} fragmented bytes", page.getPageId(), reclaimed);
        }
        return reclaimed;
    }
