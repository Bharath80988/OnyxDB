...
class ForgeQL:
    ...
    def explain(self, query_string):
        """Return the Cost-Based Optimizer (CBO) execution plan for an FQL query."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ForgeQL Terminal CLI")
    parser.add_argument("--host", default="http://localhost:8080", help="ForgeQL API Host")
    parser.add_argument("--token", default="admin-secret-key", help="Authorization Token")
    subparsers = parser.add_subparsers(dest="command")

    # FQL Command
    fql_parser = subparsers.add_parser("fql", help="Execute Forge Query Syntax string")
    fql_parser.add_argument("query", type=str, help="FQL String (e.g., 'GET users 101')")
    ...
    db = ForgeQL(host=args.host, token=args.token)

    if args.command == "fql":
        result = db.fql(args.query)
