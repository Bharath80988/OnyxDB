...
/**
 * Onyx Wire Protocol (OWP) — compact binary framing protocol for NIO TCP socket communication.
 *
 * <p>OWP Frame Layout (9-byte header + payload):
 * <pre>
 * +-------------------+------------------+-------------------------+------------------+
 * | Magic  (4 bytes)  | MsgType (1 byte) | PayloadLength (4 bytes) | Payload (N bytes)|
 * | 0x4F4E5958 "ONYX" | 0x01/0x02/0x03   | unsigned int32 BE       | UTF-8 encoded    |
 * +-------------------+------------------+-------------------------+------------------+
 * </pre>
 * </p>
 *
 * <p>Message Types:
 * <ul>
 *   <li>{@code 0x01} MSG_QUERY — Client sends an OQS string or JSON query payload.</li>
...
public class OnyxWireProtocol {
    ...
    /** Magic 4-byte header "ONYX" in big-endian hex: 0x4F4E5958. */
    ...
    /** OWP message type: client query (OQS string or JSON payload). */
