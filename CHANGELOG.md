...
All notable changes to **OnyxDB** will be documented in this file.
...
* **Onyx Wire Protocol (OWP)**: Non-blocking binary TCP socket protocol on port 8081 with a 9-byte header framing structure (`0x4F4E5958`).
* **Embedded REST API & JWT Security**: Spring Boot endpoints on port 8080 with HMAC-SHA256 JWT RBAC guards (`ADMIN` & `READ_ONLY`).
* **Onyx Studio Web IDE (`/studio`)**: Visual Drag-and-Drop Node Builder, JSON Console, Table Creator, and Telemetry.
* **Interactive REPL CLI & SDKs**: `pip install onyxdb`, `npx onyxdb`, Maven `com.onyxdb:onyxdb-core`.
