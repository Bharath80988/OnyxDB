...
export default function ExamplesPage() {
  ...
  const openInStudio = (ex) => {
    sessionStorage.setItem('onyx_studio_preset', JSON.stringify(ex.payload));
    navigate('/studio');
  };
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 text-emerald-400 text-xs font-mono mb-4 border border-white/10">
            <Sparkles className="w-3.5 h-3.5" /> OnyxDB Query Playground &amp; Complex Challenges
          </div>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/10 pb-4 mb-6 gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-emerald-400 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                      {ex.category}
                    </span>
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="bg-black/80 border border-white/10 rounded-xl overflow-hidden flex flex-col">
                  ...
                  <pre className="p-4 text-xs font-mono text-emerald-400 overflow-x-auto flex-1">
                ...
                <div className="bg-black/60 border border-white/10 rounded-xl p-4 flex flex-col justify-between">
                  <div className="text-xs font-mono font-bold uppercase text-white/40 mb-3 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-emerald-400">Node Graph</span>
                  </div>
                      ...
                      <React.Fragment key={node.id}>
                        <div className="bg-[#18181c] border border-white/20 rounded-xl p-3 text-center min-w-[130px] shadow-lg">
                          <div className="text-[10px] font-mono text-emerald-400 font-bold uppercase mb-1">
                            {node.type === 'selectNode' ? 'Select Node' : node.type === 'filterNode' ? 'Filter Node' : 'OQS Node'}
                          </div>
                  ...
                  <div className="pt-3 border-t border-white/5 flex items-center justify-between text-xs font-mono text-white/40">
                    <span>OQS: {ex.oqsEquivalent.substring(0, 32)}...</span>
                    <button
                      onClick={() => openInStudio(ex)}
