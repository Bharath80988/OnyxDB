...
export default function CliPage() {
  ...
  const [terminalOutput, setTerminalOutput] = useState([
    { type: 'sys', text: 'OnyxDB Terminal REPL v4.0.0' },
    { type: 'sys', text: 'Connected to localhost:8080 (REST) and port 8081 (OWP TCP)' },
    { type: 'sys', text: 'Type any OQS query (e.g. GET users 101) or SQL command.' },
    { type: 'sys', text: '' }
  ]);
  ...
  const handleRunCmd = async (e) => {
    ...
    const newLogs = [...terminalOutput, { type: 'cmd', text: `onyxdb> ${cmd}` }];
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 text-emerald-400 text-xs font-mono mb-4 border border-white/10">
            <Terminal className="w-3.5 h-3.5" /> Onyx Interactive Terminal CLI
          </div>
          <h1 className="text-4xl sm:text-6xl font-bold tracking-tight mb-4">
            Command-Line <span className="text-white/60">REPL Shell</span>
          </h1>
          <p className="text-lg text-white/60 leading-relaxed">
            Execute OQS queries, inspect Cost-Based Optimizer execution plans, and benchmark engine queries in real-time.
          </p>
        </div>
        ...
        <div className="bg-[#08080a] border border-white/15 rounded-2xl overflow-hidden shadow-2xl mb-16 max-w-4xl mx-auto">
          ...
          <div className="bg-black/60 px-4 py-3 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-white/50 ml-2">onyx-cli — v4.0.0</span>
            </div>
            <span className="text-[10px] font-mono text-emerald-400 uppercase">Interactive Shell</span>
          </div>
          ...
          <div className="p-6 font-mono text-xs min-h-[360px] max-h-[500px] overflow-y-auto space-y-1 bg-black/90">
            {terminalOutput.map((line, idx) => {
              ...
              if (line.type === 'res') return <pre key={idx} className="text-emerald-400 whitespace-pre-wrap">{line.text}</pre>;
          ...
          <form onSubmit={handleRunCmd} className="border-t border-white/10 bg-black/60 p-3 flex items-center gap-2">
            <span className="text-emerald-400 font-mono text-xs font-bold pl-2">onyxdb&gt;</span>
            <input
              type="text"
              value={inputCmd}
        ...
        {/* OQS Command Reference */}
        <div className="bg-[#121214] border border-white/10 rounded-2xl p-8 max-w-4xl mx-auto">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
            <Code className="w-5 h-5 text-emerald-400" /> Onyx Query Syntax (OQS) Reference
          </h2>
