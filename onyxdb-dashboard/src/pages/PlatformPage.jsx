...
const PLATFORM_ITEMS = [
  {
    ...
    tag: "NODE.JS PACKAGE",
    desc: "Bootstrap an embedded OnyxDB server directly via npx or import as a dependency into Express & Next.js backends.",
    longDesc: "The NPM wrapper ('npx onyxdb' or 'npm install onyxdb') contains the embedded OnyxDB standalone JAR and Node.js process manager. It listens on HTTP port 8080 and TCP socket port 8081.",
    file: "index.js / package.json",
    specs: [
      { label: "NPM Package Name", value: "onyxdb" },
      { label: "Binary Entrypoint", value: "npx onyxdb" },
...
console.log(res.data);`
  },
  {
    ...
    longDesc: "The Python package ('pip install onyxdb') includes an interactive terminal shell (onyxdb start) and native Python bindings for non-blocking HTTP and TCP socket querying.",
    file: "pip-wrapper/setup.py",
    specs: [
      { label: "PyPI Package", value: "pip install onyxdb" },
      { label: "CLI Launcher", value: "onyxdb start" },
...
print(response.json())`
  },
  {
    ...
    longDesc: "OnyxDB is compiled as an independent Java JAR (onyxdb-api-0.2.0.jar) using Maven. It embeds Tomcat on port 8080 and a Round-Robin Multi-Reactor TCP server on port 8081.",
    file: "onyxdb-api/pom.xml",
    specs: [
      { label: "Artifact ID", value: "com.onyxdb:onyxdb-api" },
...
<dependency>
  ...
  },
  {
    ...
    longDesc: "By leveraging Operating System virtual memory page mapping, OnyxDB bypasses Java garbage collector pauses and user-space buffer copying during heavy write operations.",
...
// Direct page slot lookup in off-heap virtual memory`
  ...
  {
    id: "owp",
    title: "Onyx Wire Protocol (OWP) Binary TCP",
    tag: "NETWORKING",
    desc: "High-throughput binary socket protocol on TCP port 8081 with a 9-byte header framing structure (0x4F4E5958).",
    longDesc: "Onyx Wire Protocol (OWP) provides raw TCP binary socket communication, eliminating HTTP header parsing overhead. Features a 4-byte magic header, 1-byte message type, and 4-byte payload length.",
...
[ JSON Payload Payload Bytes... ]`
  },
  {
    ...
    longDesc: "When executing OQS queries, QueryOptimizer evaluates selectivity metrics for primary B+ Trees vs secondary indexes, outputting detailed CBO cost estimates.",
...
{
  "oqs": "EXPLAIN FIND orders WHERE status = 'COMPLETED'"
}`
  }
];
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 text-emerald-400 text-xs font-mono mb-4 border border-white/10">
            <Server className="w-3.5 h-3.5" /> OnyxDB Multi-Language Platform
          </div>
          ...
          {PLATFORM_ITEMS.map((item, idx) => (
            ...
            >
              ...
              <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono">
                ...
                <span className="text-emerald-400 font-bold group-hover:underline">View Specs &rarr;</span>
              </div>
            </motion.div>
          ))}
        </div>
