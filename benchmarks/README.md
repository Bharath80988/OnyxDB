# OnyxDB Benchmarks & Performance Metrics

This directory contains benchmark suites, execution methodologies, and performance profiles comparing **OnyxDB v0.1.0** against relational and embedded storage engines.
...
| Workload Benchmark | OnyxDB v0.1.0 | PostgreSQL 16 | SQLite 3.42 | Engine Mechanism |
|---|---|---|---|---|
...
```bash
cd onyxdb-core
mvn test-compile exec:java -Dexec.mainClass="com.onyxdb.core.benchmark.StorageEngineBenchmark"
```

### 2. Run HNSW Vector Distance Benchmark
```bash
mvn test-compile exec:java -Dexec.mainClass="com.onyxdb.core.benchmark.HnswVectorBenchmark"
```

### 3. Run TCP Socket Protocol (OWP) Benchmark
```bash
mvn test-compile exec:java -Dexec.mainClass="com.onyxdb.api.benchmark.OwpProtocolBenchmark"
```
